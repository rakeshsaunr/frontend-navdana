...
const Header = () => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (Array.isArray(response.data.categories)) {
          // Only keep active categories except "All Products"
          const activeCategories = response.data.categories
            .filter((cat) => cat.isActive && cat.name !== "All Products");

          // Add "All Products" at the start
  ...
  const handleCategoryClick = (category) => {
    if (category._id === "all-products") {
      navigate("/all-products"); // special page for all products
    } else {
      navigate(`/collection-pages/${category._id}`); // normal category pages
    }
  };
